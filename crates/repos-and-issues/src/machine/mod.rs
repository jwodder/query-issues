use crate::queries::{GetIssues, GetLabels, GetOwnerRepos, Issue, Repository};
use gqlient::{BatchPaginator, Id, JsonMap, QueryMachine, QueryPayload};
use serde::Serialize;
use std::collections::HashMap;
use std::num::NonZeroUsize;

/// A [`QueryMachine`] for fetching repositories owned by given users and/or
/// organizations along with any open issues within them
///
...
pub(crate) struct ReposAndIssues<S> {
    /// Internal state
    state: State,
...
impl ReposAndIssues<()> {
    /// Create a new `ReposAndIssues` instance that will fetch open issues in
    /// repositories owned by any owners listed in `owners`
    pub(crate) fn new(owners: Vec<String>, limits: QueryLimits) -> ReposAndIssues<()> {
        ReposAndIssues {
...
impl<S> ReposAndIssues<S> {
    /// Set the [`EventSubscriber`] that the `ReposAndIssues` will report
    /// transition events to
    pub(crate) fn with_subscriber<S2>(self, subscriber: S2) -> ReposAndIssues<S2> {
        let ReposAndIssues {
            state,
            shared: Shared {
                limits, results, ..
...
impl<S: EventSubscriber> QueryMachine for ReposAndIssues<S> {
    type Output = Output;
    ...
    fn get_output(&mut self) -> Vec<Output> {
        self.shared.results.drain(..).collect()
    }
}

/// Data on the `ReposAndIssues` instance that is accessed by the methods of
...
/// Behavior common to all states of the `ReposAndIssues` state machine
...
/// Internal states of the `ReposAndIssues` state machine
...
impl MachineState for FetchRepos {
    ...
    fn handle_response<S: EventSubscriber>(
            ...
            .flat_map(|pr| pr.items)
        {
            ...
            shared.results.push(Output::Repository(repo));
        }
        Ok(())
    }
}
    ...
    fn handle_response<S: EventSubscriber>(
            ...
            .flat_map(|pr| pr.items)
        {
            ...
            if let Some(q) = iwl.more_labels_query(shared.limits.label_page_size) {
                ...
                shared.results.push(Output::Issue(iwl.issue));
            }
        }
        Ok(())
    }
...
impl MachineState for FetchLabels {
    fn get_next_query<S: EventSubscriber>(
        ...
        if let query @ Some(_) = self.submachine.get_next_query() {
            ...
            shared
                .results
                .extend(self.issues_needing_labels.into_values().map(Output::Issue));
            Done::start(shared)
        }
    }
...
/// and fetched resources from an `ReposAndIssues`
...
///
/// During the lifetime of an `ReposAndIssues`, events are always passed to an
