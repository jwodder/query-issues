mod machine;
mod queries;
use crate::machine::{Event, EventSubscriber, QueryLimits, ReposAndIssues};
use anyhow::Context;
use clap::Parser;
...
/// Measure time to fetch repositories and open GitHub issues via GraphQL
...
struct Arguments {
    ...
    /// GitHub owners/organizations of repositories to fetch
    #[arg(required = true)]
    owners: Vec<String>,
}
...
fn main() -> anyhow::Result<()> {
    ...
    let machine =
        ReposAndIssues::new(args.owners.clone(), parameters).with_subscriber(&mut recorder);
    let resources = client.run(machine).collect::<Result<Vec<_>, _>>()?;
    ...
    if let Some(outfile) = args.outfile {
        ...
        fp.write_json_lines(resources)
            .context("failed to dump resources")?;
