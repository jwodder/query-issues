...
repositories belonging to a collection of users/organizations.
...
Fetch information on open issues in the repositories owned by the given
owner(s).
...
`orgs-with-issues` requires a GitHub access token with appropriate permissions
